...
parser.add_argument('--dataRoot', default='testimage', help='path to real images')
outfilename = 'cis_output'
os.makedirs(outfilename, exist_ok=True)

parser.add_argument('--experiment0', default='models/check_cascade0_w320_h240', help='the path to the model of first cascade')
parser.add_argument('--experimentLight0', default='models/check_cascadeLight0_sg12_offset1.0', help='the path to the model of first cascade')
parser.add_argument('--experimentBS0', default='models/checkBs_cascade0_w320_h240', help='the path to the model of bilateral solver')
parser.add_argument('--experiment1', default='models/check_cascade1_w320_h240', help='the path to the model of second cascade')
parser.add_argument('--experimentLight1', default='models/check_cascadeLight1_sg12_offset1.0', help='the path to the model of second cascade')
parser.add_argument('--experimentBS1', default='models/checkBs_cascade1_w320_h240', help='the path to the model of second bilateral solver')
...
parser.add_argument('--isLight', action='store_true', default=True, help='whether to predict lightig')
parser.add_argument('--isBS', action='store_true', default=True, help='whether to use bilateral solver')
...
imBatchSmall = torch.FloatTensor(opt.batchSize, 3, opt.envRow, opt.envCol)
...
for imName in imList:
    ...
    for n in range(0, opt.level):
        albedoNames.append(osp.join(outfilename, imId.replace('.jpg', '_albedo%d.npy' % n)))
        albedoImNames.append(osp.join(outfilename, imId.replace('.jpg', '_albedo%d.png' % n)))
        normalNames.append(osp.join(outfilename, imId.replace('.jpg', '_normal%d.npy' % n)))
        normalImNames.append(osp.join(outfilename, imId.replace('.jpg', '_normal%d.png' % n)))
        roughNames.append(osp.join(outfilename, imId.replace('.jpg', '_rough%d.npy' % n)))
        roughImNames.append(osp.join(outfilename, imId.replace('.jpg', '_rough%d.png' % n)))
        depthNames.append(osp.join(outfilename, imId.replace('.jpg', '_depth%d.npy' % n)))
        depthImNames.append(osp.join(outfilename, imId.replace('.jpg', '_depth%d.png' % n)))
        ...
        envmapsPredSGNames.append(osp.join(outfilename, imId.replace('.jpg', '_envmapSG%d.npy' % n)))
        shadingNames.append(osp.join(outfilename, imId.replace('.jpg', '_shading%d.png' % n)))
        envmapPredNames.append(osp.join(outfilename, imId.replace('.jpg', '_envmap%d.npz' % n)))
        envmapPredImNames.append(osp.join(outfilename, imId.replace('.jpg', '_envmap%d.png' % n)))
        renderedNames.append(osp.join(outfilename, imId.replace('.jpg', '_rendered%d.npy' % n)))
        renderedImNames.append(osp.join(outfilename, imId.replace('.jpg', '_rendered%d.png' % n)))

        cLightNames.append(osp.join(outfilename, imId.replace('.jpg', '_cLight%d.mat' % n)))
    ...
    renderLayer = models.renderingLayer(isCuda=opt.cuda, imWidth=newEnvWidth, imHeight=newEnvHeight, fov=fov, envWidth=opt.envWidth, envHeight=opt.envHeight)

    output2env = models.output2env(isCuda=opt.cuda, envWidth=opt.envWidth, envHeight=opt.envHeight, SGNum=opt.SGNum)
