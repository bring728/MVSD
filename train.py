...
# output = '~/VSCodeProjects/MPR_openrooms_2/mpi-640'
...
def main():
    ...
    experiment = f'{output}/mpi_{normlayer}_{grouptype}_{lr}_{batchsize}'
    os.makedirs(experiment, exist_ok=True)

    with open(os.path.join(experiment, "config.yml"), "w") as f:
        f.write(cfg.dump())  # cfg, f, default_flow_style=False)

    # batchSize = torch.cuda.device_count()
...
def train(gpu, ngpus_per_node, cfg, experiment, id):
    ...
    num_view_all = cfg.num_view_all
    interp = cfg.interp
    num_planes = cfg.num_planes
    batchsize = int(cfg.batchsize / ngpus_per_node)
    ...
    ####################################
    pinned = True
    _Openrooms_dataset = Openrooms_dataset(dataRoot, num_view_all=num_view_all, num_planes=num_planes, phase=phase)
    _Openrooms_sampler = torch.utils.data.distributed.DistributedSampler(_Openrooms_dataset)
    openrooms_loader = torch.utils.data.DataLoader(_Openrooms_dataset, batch_size=batchsize, shuffle=False, num_workers=num_workers, pin_memory=pinned,
                                             sampler=_Openrooms_sampler)

    # Create IBRNet model
    mvsd_model = MVSDModel(cfg)
    # create projector
    ...
    while global_step < model.start_step + args.n_iters + 1:
        ...
        for train_data in train_loader:
            ...
            ray_batch = ray_sampler.random_sample(N_rand, sample_mode=args.sample_mode, center_ratio=args.center_ratio, )
