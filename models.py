...
class MultiViewModel(object):
    def __init__(self, cfg, gpu, experiment, load_opt=True, load_scheduler=True, phase='TRAIN', is_DDP=True):
        ...
        if self.is_DDP:
            ...
            if self.mode == 'SVL' or self.mode == 'finetune':
                ...
                self.VSG_Net = torch.nn.parallel.DistributedDataParallel(self.VSG_Net, device_ids=[gpu], )
    ...
    def save_model(self, filename):
        ...
        if self.mode == 'SVL' or self.mode == 'finetune':
            ...
            to_save['VSG_Net'] = de_parallel(self.VSG_Net).state_dict()
    ...
    def load_model(self, filename, load_opt=True, load_scheduler=True):
        ...
        if 'GL_Net' in to_load:
            ...
            self.VSG_Net.load_state_dict(to_load['VSG_Net'])
            print('GL, VSG Net loaded from ', filename)
