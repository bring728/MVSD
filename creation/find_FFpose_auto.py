...
def main():
    ...
    for xml_type in ['xml1', ]:
        ...
        for xml, out_dir in zip(xml_list, outdir_list):
            ...
            if not osp.isfile(pre_pushed_file):
                z += 1
                continue
            ...
            with open(pre_pushed_file, 'r') as fIn:
                pre_pushed_txt = fIn.readlines()
            if len(pre_pushed_txt) != 1:
                raise Exception('pushed error')
            ...
            f_tmp.close()
            pre_pushed_txt = pre_pushed_txt[0].split(' ')
