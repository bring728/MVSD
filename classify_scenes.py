...
root_lemon = '/home/happily/Data/lemon/'
root_org = '/home/happily/Data/OpenRoomsDataset/data/rendering/data_FF_10_640/'
dir_list = os.listdir(root_lemon)
for dir in dir_list:
    scene_list = os.listdir(root_lemon + dir)
    for scene in scene_list:
        im_lemon_name = root_lemon + dir + '/' + scene + '/im_1.rgbe'
        if not os.path.exists(im_lemon_name):
            continue
        ...
        scale = get_hdr_scale(im, seg, 'TEST')
        im_lemon = img_rgb2bgr(np.clip(im * scale, 0, 1.0)).transpose((1,2,0))
